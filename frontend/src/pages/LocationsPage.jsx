    ...
    <div className="loc-page">
      <header className="loc-hero-head">
        ...
        <p className="loc-subtitle">
          Fedezd fel az egyetem ikonikus pontjait. Válassz egy helyszínt, nézd
          meg a részleteit, majd teleportálj a 3D modell megfelelő pontjára.
        </p>
      </header>
      ...
      <section className="loc-quick-steps" aria-label="Használati útmutató">
        ...
        <article className="loc-step-card">
          ...
          <p>
            Indítsd el a modellt, és teleportálj közvetlenül a kiválasztott
            pontra.
          </p>
        </article>
      </section>
          ...
          <section className="loc-admin-create">
            ...
            <form onSubmit={handleCreateLocation} className="loc-create-form">
              ...
              >
                <option value="">
                  {objectsLoading
                    ? "Location objectek betöltése..."
                    : "Válassz location objectet"}
                </option>
                {locationObjects.map((obj) => {
                  const objectName = getObjectName(obj);
                  if (!objectName) return null;
            ...
            {!objectsLoading && locationObjects.length === 0 && (
              <p className="loc-error">
                Nincs elérhető location object a választáshoz.
              </p>
            )}
        ...
        {!locationsLoading && !locationsError && selectedLocation && (
          <section className="loc-detail-view">
            ...
            <article className="loc-detail-card">
              ...
              <p className="loc-detail-description">
                    ...
                    <div className="loc-card">
                      {isAdmin && editingLoc?.loc_id === loc.loc_id ? (
                        ...
                        >
                          ...
                          >
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setEditButtonLocation(e.target.value)
                              }
                              disabled={
                                objectsLoading ||
                            ...
                            >
                              ...
                              {editButtonLocation &&
                                !locationObjects.some(
                                  (obj) =>
                                    getObjectName(obj) === editButtonLocation,
                                ) && (
                                  <option value={editButtonLocation}>
                                    {editButtonLocation} (jelenlegi)
                                  </option>
                                )}
                              {locationObjects.map((obj) => {
                                const objectName = getObjectName(obj);
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setEditInformation(e.target.value)
                              }
                              placeholder="Leírás"
                              rows={4}
                            />
                            {editError && (
                              <p className="loc-error">{editError}</p>
                            )}
                          </div>
                        ...
                        <>
                          <div>
                            <h3>{loc.name}</h3>
                            <p className="loc-card-info loc-card-info-preview">
                              {loc.information}
                            </p>
                          </div>
                            ...
                            {isAdmin && (
                              <div className="loc-card-admin-actions">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleDeleteLocation(loc.loc_id)
                                  }
                                  disabled={deletingId === loc.loc_id}
                                  className="loc-btn-delete"
                                >
                                  {deletingId === loc.loc_id
                                    ? "Törlés..."
                                    : "Törlés"}
                                </button>
                              </div>
